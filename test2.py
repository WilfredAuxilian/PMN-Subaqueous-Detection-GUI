...
class Toplevel1:
    def __init__(self, top=None):
        ...
        self.Class_Label_Fish = tk.Label(self.Labelframe5, text='Fish: 0')
        self.Class_Label_Fish.place(relx=0.05, rely=0.85, height=20, width=100)
        self.Class_Label_Fish.configure(background="#efefef", font="-family {Segoe UI} -size 9", foreground="#000000")

        self.Class_Label_PMN = tk.Label(self.Labelframe5, text='PMN: 0')
        self.Class_Label_PMN.place(relx=0.25, rely=0.85, height=20, width=100)
        self.Class_Label_PMN.configure(background="#efefef", font="-family {Segoe UI} -size 9", foreground="#000000")

        self.Class_Label_Coral = tk.Label(self.Labelframe5, text='Coral Reef: 0')
        self.Class_Label_Coral.place(relx=0.45, rely=0.85, height=20, width=100)
        self.Class_Label_Coral.configure(background="#efefef", font="-family {Segoe UI} -size 9", foreground="#000000")

        self.Class_Label_Algae = tk.Label(self.Labelframe5, text='Algae: 0')
        self.Class_Label_Algae.place(relx=0.65, rely=0.85, height=20, width=100)
        self.Class_Label_Algae.configure(background="#efefef", font="-family {Segoe UI} -size 9", foreground="#000000")
        ...
        self.Label3 = tk.Label(self.Labelframe6, text='''Distance between Cam & Seabed''')
        ...
        self.Label4 = tk.Label(self.Labelframe6, text='''Number of Nodules Detected''')
    ...
    def apply_changes(self):
        ...
        if current_image is not None:
            ...
            focal_length = self.Entry1.get() or "0"

            processed = current_image.copy()
            counts = {"Fish": 0, "PMN": 0, "Coral Reef": 0, "Algae": 0}
                    ...
                    if model_path.endswith('.pt'):  # YOLO model
                        ...
                        for result in results:
                            for box in result.boxes:
                                ...
                                if dataset == "Fish" and label.lower() == "goldfish":
                                    counts["Fish"] += 1
                                elif dataset == "PMN":
                                    counts["PMN"] += 1
                                elif dataset == "Coral Reef":
                                    counts["Coral Reef"] += 1
                                elif dataset == "Algae":
                                    counts["Algae"] += 1
            ...
            self.Entry6.insert(0, self.calculate_distance(focal_length))
            self.Entry4.delete(0, tk.END)
            self.Entry4.insert(0, str(counts["Fish"] + counts["Coral Reef"]))  # Fauna
            self.Entry5.delete(0, tk.END)
            self.Entry5.insert(0, str(counts["Coral Reef"]))  # Flora (assuming coral as flora)
            self.Entry3.delete(0, tk.END)
            self.Entry3.insert(0, str(counts["PMN"]))
            self.Entry7.delete(0, tk.END)
            self.Entry7.insert(0, str(counts["Algae"]))

            # Update visualization class counts
            self.Class_Label_Fish.configure(text=f'Fish: {counts["Fish"]}')
            self.Class_Label_PMN.configure(text=f'PMN: {counts["PMN"]}')
            self.Class_Label_Coral.configure(text=f'Coral Reef: {counts["Coral Reef"]}')
            self.Class_Label_Algae.configure(text=f'Algae: {counts["Algae"]}')
    ...
    def select_model(self, dataset):
        if getattr(self, f"che{51 if dataset == 'Algae' else 47 if dataset == 'PMN' else 48 if dataset == 'Fish' else 49}").get():
            ...
            if model_path:
                ...
                label.configure(text=os.path.basename(model_path))
            else:
