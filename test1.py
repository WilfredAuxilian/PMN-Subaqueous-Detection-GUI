...
class Toplevel1:
    def __init__(self, top=None):
        ...
        self.Entry1.insert(0, "Enter Focal Length (mm)")
    ...
    def browse_image(self):
        ...
        if file_path:
            ...
            print(f"Image displayed: {file_path}")
    ...
    def browse_video(self):
        ...
        if not ret:
            ...
            messagebox.showerror("Video Error", f"Could not read frame from: {file_path}")
            cap.release()
            return
        ...
        try:
            ...
            photo = ImageTk.PhotoImage(image)
            self.Display_Window.configure(image=photo)
            self.Display_Window.image = photo  # Keep reference to avoid garbage collection
            print("Video frame displayed successfully")
        except Exception as e:
            print(f"Error processing video frame: {str(e)}")
            messagebox.showerror("Processing Error", f"Error displaying video frame: {str(e)}")
    ...
    def calculate_results(self, image, focal_length=None):
        ...
        pixel_to_meter = 0.01
        frame_area = (height * pixel_to_meter) * (width * pixel_to_meter)
        if focal_length and focal_length != "Enter Focal Length (mm)":
            try:
                focal_length = float(focal_length) / 1000
                assumed_object_size = 1.0
                object_size_in_image = max(height, width) * pixel_to_meter / 10
    ...
    def apply_changes(self):
        ...
        self.Entry2.insert(0, str(total_counts["flora"]))
        self.Entry5.insert(0, f"{frame_area:.2f} m²")
        self.Entry3.insert(0, str(total_counts["nodules"]))
        self.Entry6.insert(0, f"{distance:.2f} m")
        self.Entry4.insert(0, str(total_counts["fauna"]))
        self.Entry7.insert(0, str(total_counts["algae"]))
